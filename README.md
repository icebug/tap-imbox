...
Create tests within the `tap_imbox/tests` subfolder and
  then run:
...
You can also test the `tap-imbox` CLI interface directly using `poetry run`:
...
meltano elt tap-imbox target-jsonl
```
